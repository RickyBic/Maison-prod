...
public class HibernateDao {
    ...
    public <T> List<T> findAll(Class<T> tClass, String orderBy, boolean orderAsc) {
        Session session = sessionFactory.openSession();
        Order order = (orderAsc) ? Order.asc(orderBy) : Order.desc(orderBy);
        List<T> results = session.createCriteria(tClass).addOrder(order).list();
        session.close();
        return results;
    }
    ...
    public <T> List<T> findWhere(T entity, String orderBy, boolean orderAsc) {
        Session session = sessionFactory.openSession();
        Order order = (orderAsc) ? Order.asc(orderBy) : Order.desc(orderBy);
        Example example = Example.create(entity).ignoreCase();
        List<T> results = session.createCriteria(entity.getClass()).addOrder(order).add(example).list();
        session.close();
        return results;
    }
